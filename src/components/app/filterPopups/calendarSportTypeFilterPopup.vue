<template>
    <f7-popup class="calendarSportTypeFilter-popup">
      <f7-page>
        <f7-searchbar v-if="renderSearch"
          search-container=".calendar-sport-type-virtual-list"
          search-item="li"
        ...
        <f7-list
          ...
          inset-md
          dividers-ios
          class="searchbar-found calendar-sport-type-virtual-list"
          medial-list
          virtual-list
          :virtual-list-params="{
            items,
  ...
  export default {
    ...
    data() {
      ...
      const selected = useStore("currentCalendarSportsType")
      const items = filtersData.calendarSportType;
      let i = 0;
      items.forEach(item => {
        item.index = i + 1;
        i++;

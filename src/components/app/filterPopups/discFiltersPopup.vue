<template>
    <f7-popup class="discFilter-popup">
      <f7-page>
        <f7-searchbar v-if="renderSearch"
          search-container=".disc-virtual-list"
          search-item="li"
        ...
        <f7-list
          ...
          inset-md
          dividers-ios
          class="searchbar-found disc-virtual-list"
          medial-list
          virtual-list
          :virtual-list-params="{
            items,
  ...
  export default {
    ...
    data() {
      ...
      const selected = useStore("currentDisciplineId")
      const items = filtersData.disciplineInfo;
      let i = 0;
      items.forEach(item => {
        item.index = i + 1;
        i++;

<template>
    <f7-popup class="programFilter-popup">
      <f7-page>
        <f7-searchbar v-if="renderSearch"
          search-container=".program-virtual-list"
          search-item="li"
        ...
        <f7-list
          ...
          inset-md
          dividers-ios
          class="searchbar-found program-virtual-list"
          medial-list
          virtual-list
          :virtual-list-params="{
            items,
  ...
  export default {
    ...
    data() {
      ...
      const selected = useStore("currentProgramId")
      const items = filtersData.programInfo;
      let i = 0;
      items.forEach(item => {
        item.index = i + 1;
        i++;

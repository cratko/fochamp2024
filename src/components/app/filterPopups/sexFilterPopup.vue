<template>
    <f7-popup class="sexFilter-popup">
      <f7-page>
        <f7-searchbar v-if="renderSearch"
          search-container=".sex-virtual-list"
          search-item="li"
        ...
        <f7-list
          ...
          inset-md
          dividers-ios
          class="searchbar-found sex-virtual-list"
          medial-list
          virtual-list
          :virtual-list-params="{
            items,
  ...
  export default {
    ...
    data() {
      ...
      const selected = useStore("currentSexId")
      const items = filtersData.sexCategory;
      let i = 0;
      items.forEach(item => {
        item.index = i + 1;
        i++;
    ...
    methods: {
      searchAll(query, items) {
        ...
        for (let i = 0; i < items.length; i += 1) {
          if (items[i].sex.toLowerCase().includes(query.toLowerCase()) || query.trim() === '') {
            found.push(i);
          }
        }

<template>
    <f7-popup class="teamFilter-popup">
      <f7-page>
        <f7-searchbar v-if="renderSearch"
          search-container=".team-virtual-list"
          search-item="li"
        ...
        <f7-list
          ...
          inset-md
          dividers-ios
          class="searchbar-found team-virtual-list"
          medial-list
          virtual-list
          :virtual-list-params="{
            items,
  ...
  export default {
    ...
    data() {
      ...
      const selected = useStore("currentTeam")
      const items = filtersData.teamInfo;
      let i = 0;
      items.forEach(item => {
        item.index = i + 1;
        i++;
